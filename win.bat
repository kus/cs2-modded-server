...
start /wait %ROOT_DIR%server\game\bin\win64\cs2.exe -dedicated -console -debug -condebug -usercon +game_type 0 +game_mode 0 +mapgroup mg_active +map de_dust2 +hostport %csgo_port% -ip 0.0.0.0 +net_public_adr %ip_internet% -tickrate %csgo_tickrate% -maxplayers_override %csgo_players% -authkey %csgo_api_key%
