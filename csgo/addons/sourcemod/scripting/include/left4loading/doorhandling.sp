 ...
 *  Left 4 Loading
 *
 *  File:			doorhandling.sp
 *  Type:			Module
 *  Description:	Handles checkpoint doors.
 *
...
static	const	String:	CLASSNAME_CHECKPOINT[]			= "prop_door_rotating_checkpoint";
static	const	String:	SERVERCLASS_CHECKPOINT[]		= "CPropDoorRotatingCheckpoint";
static	const	String:	NETPROP_SPAWNFLAGS[]			= "m_spawnflags";
static	const	String:	NETPROP_LOCKED[]				= "m_bLocked"; // Only the exit door will be locked
...
static	const	String:	FORCE_START_TIME_CVAR[]			= "versus_force_start_time"; // Changing this cvar will prevent director from auto open the checkpoint door
static	const			FORCE_START_TIME				= 9999999; // Any random large value will do

static					g_iCheckpointDoor				= 0; // Checkpoint door entity index
...
static	const	String:	DIRECTOR_FORCE_VS_START[]		= "director_force_versus_start";

static			Float:	g_fCheckIfDoorIsStuck			= 5.0; /* Sometimes the door bugs out, luckily for us we can just remove it. 
																* Wait this many seconds after forcing it to see if the door is stuck 
																* and remove it if so. */
...
public _DoorHandling_OnPluginStart()
{
	...
	RegAdminCmdEx("removedoor", _DH_RemoveDoor_Command, ADMFLAG_GENERIC);
}
...
public Action:_DH_OnReadyUpStart_Timer(Handle:timer)
{
	...
	if (door && (IsCoop() || IsVersus()) && !IsSurvival())
	{
