...
void CreateNatives()
{
	...
	// void AddModelHook(ModelHookCallback callback)
	CreateNative("CustomWeapon.AddSoundHook", Native_AddSoundHook);
	
	// void RemoveModelHook(ModelHookCallback callback)
	CreateNative("CustomWeapon.RemoveSoundHook", Native_RemoveSoundHook);
}
...
any Native_AddSoundHook(Handle plugin, int numParams)
{
	// Param 1: 'CustomWeapon' [this]
	ValidateCustomWeapon(view_as<CustomWeapon>(GetNativeCell(1)));
	
	g_SoundHook.AddFunction(plugin, GetNativeFunction(2));
	
	return 0;
}

any Native_RemoveSoundHook(Handle plugin, int numParams)
{
	// Param 1: 'CustomWeapon' [this]
	ValidateCustomWeapon(view_as<CustomWeapon>(GetNativeCell(1)));
	
	g_SoundHook.RemoveFunction(plugin, GetNativeFunction(2));
